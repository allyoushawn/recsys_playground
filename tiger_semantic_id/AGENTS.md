...
### Raw Data Acquisition for Investigation

**Investigation Notebook:** `notebooks/tiger_semantic_id/investigate_llm_data.ipynb`

**⚠️ CRITICAL REQUIREMENT:** This notebook MUST be run in Google Colab with tiger_semantic_id artifacts in Google Drive. It will NOT work in local Jupyter environments because it requires:
- Google Drive mount for persistent artifact storage
- Access to previously generated RQ-VAE artifacts from `TIGER_SemanticID.ipynb`
- Access to LLM artifacts from `TIGER_SemanticID_LLM_finetune.ipynb`
...
# Full path: /content/drive/MyDrive/colab/tiger_semantic_id/raw_data/meta_Video_Games.jsonl.gz
```
...
**Key features:**
- Uses `urllib.request.urlretrieve()` for direct download from UCSD repository
- Stores in Google Drive for persistence across Colab sessions
- Skips download if file already exists (checks with `meta_file.exists()`)
- File size: ~50MB compressed
- Download time: ~1-3 minutes depending on connection speed

**Row Counting (Fast Verification):**
```python
# Count rows without parsing JSON (fast verification method)
raw_count = 0
...
# Verify count matches expected: ~137,269 items
```

**Processing approach:**
- Line-by-line counting without JSON parsing (fast verification)
- Uses `gzip.open(meta_file, 'rt')` to read compressed file directly
- Progress indicator every 10,000 rows for user feedback
- Counts total items: 137,269 (matches Amazon 2023 reference)

#### Phase 1: Comparing Raw vs Filtered Data

**Investigation Workflow:**

1. **Phase 0: Download and count raw unfiltered data**
   - Downloads `meta_Video_Games.jsonl.gz` from UCSD repository
   - Counts rows without parsing: **137,269 items** (truly unfiltered)
   - Stores in Google Drive for reuse

2. **Phase 1: Load filtered data from RQ-VAE pipeline**
   - Loads `item_metadata.json` from TIGER_SemanticID.ipynb
   - Path: `/content/drive/MyDrive/colab/tiger_semantic_id/rq_vae_building/artifacts/item_metadata.json`
   - This file contains items AFTER user interaction filtering
   - Count: **12,101 items** (heavily filtered)

3. **Phase 1: Calculate filtering loss**
   - Comparison: 137,269 (raw) → 12,101 (filtered)
   - **Loss: 125,168 items removed (91% filtering rate)**
   - Root cause identified: `min_user_interactions ≥ 5` filter

4. **Phase 2: Analyze metadata quality**
   - Checks field availability (title, description, features, category_leaf)
   - Verifies LLM filtering criteria:
     - Title length ≥ 20 characters
     - Description length ≥ 100 characters
   - Predicts final LLM training data size after these filters

5. **Phase 3: Analyze co-occurrence sparsity**
   - Loads user sequences from LLM artifacts
   - Analyzes sequence length distribution
   - Investigates why Type E (co-purchase) examples are sparse

6. **Phase 4: Consolidated findings and recommendations**
   - Identifies user interaction filter as primary cause
   - Provides actionable solutions with impact estimates

#### Key Distinction: Raw vs Filtered vs LLM-Ready Data

**Three data stages tracked by investigation:**

1. **Raw data (Phase 0):** `meta_Video_Games.jsonl.gz`
   - Source: Amazon 2023 dataset repository (UCSD)
   - Count: 137,269 items
   - State: Truly unfiltered from source
   - Contains: ALL Video Games products regardless of interactions

2. **Filtered data (Phase 1):** `item_metadata.json`
   - Source: Generated by `TIGER_SemanticID.ipynb` via `data_preparation.ipynb`
   - Count: 12,101 items
   - State: After user interaction filter (`min_user_interactions ≥ 5`)
   - Contains: Only items with ≥5 user interactions
   - **91% of items removed at this stage**

3. **LLM-ready data (Phase 2):** Final training examples
   - Source: Generated by `TIGER_SemanticID_LLM_finetune.ipynb`
   - Count: ~42,100 items (estimated from filtered set)
   - State: After title/description length filters
   - Contains: Items passing:
     - Title length ≥ 20 characters
     - Description length ≥ 100 characters
   - Used to generate 2.66M training examples (416K for Type C)

**Why this clear separation matters:**

This three-stage tracking allows the investigation to pinpoint exactly WHERE items are lost in the pipeline:
- **Stage 1 → 2:** 91% loss due to user interaction filter (PRIMARY bottleneck)
- **Stage 2 → 3:** ~50% loss due to metadata quality filters (SECONDARY bottleneck)

Without downloading the truly raw Amazon 2023 data (Stage 1), the investigation would incorrectly assume the filtered data (Stage 2) was the baseline, missing the massive 91% filtering loss that causes the training data gap.

#### Results and Impact

**Filtering Chain Identified:**
```
