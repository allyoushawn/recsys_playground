...
from mmoe_model import MMoEModel  # noqa: E402


@dataclass
class Config:
    data_dir: str
    out_dir: str = "./runs/mmoe_census"
    epochs: int = 15
    batch_size: int = 4096
    num_workers: int = 4
    lr: float = 2e-3
...
def parse_args() -> Config:
    p = argparse.ArgumentParser(description="Train MMoE on Census-Income (KDD)")
    p.add_argument("--data_dir", type=str, required=True)
    p.add_argument("--out_dir", type=str, default="./runs/mmoe_census")
    ...
    p.add_argument("--num_levels", type=int, default=1)
...
def compute_pos_weight(loader: DataLoader) -> Tuple[float, float]:
    inc_pos = inc_neg = nm_pos = nm_neg = 0
    for batch in loader:
        yi = batch["y_income"].view(-1).to(torch.int)
        yn = batch["y_never_married"].view(-1).to(torch.int)
        inc_pos += int((yi == 1).sum()); inc_neg += int((yi == 0).sum())
        nm_pos += int((yn == 1).sum()); nm_neg += int((yn == 0).sum())
...
def train_epoch(
    ...
    for batch in loader:
        ...
        ac = _amp.autocast(device_type=device_type, enabled=mixed_precision) if _USE_NEW_AMP else _amp.autocast(enabled=mixed_precision)
        with ac:
            logit_i, logit_n = model(x)
            if pos_weights[0] is not None:
                loss_i = nn.BCEWithLogitsLoss(pos_weight=torch.tensor([float(pos_weights[0])], device=device))(logit_i, yi)
            else:
                loss_i = nn.BCEWithLogitsLoss()(logit_i, yi)
            if pos_weights[1] is not None:
                loss_n = nn.BCEWithLogitsLoss(pos_weight=torch.tensor([float(pos_weights[1])], device=device))(logit_n, yn)
            else:
                loss_n = nn.BCEWithLogitsLoss()(logit_n, yn)
            loss = w_income * loss_i + w_nm * loss_n

        if scaler is not None and mixed_precision and device == "cuda":
...
def main() -> None:
    ...
    scaler = _amp.GradScaler("cuda", enabled=cfg.mixed_precision and device == "cuda") if _USE_NEW_AMP else _amp.GradScaler(enabled=cfg.mixed_precision and device == "cuda")
    ...
    for epoch in range(1, cfg.epochs + 1):
        tr = train_epoch(
            model, loaders["train"], device, optimizer, scaler,
            cfg.w_income, cfg.w_never_married, pos_w, cfg.grad_clip, cfg.mixed_precision,
        )
        ...
        line = {"epoch": epoch, "train": tr, "val": va, "lr": optimizer.param_groups[0]["lr"]}
    ...
    report = {"test": te, "best_val_combined_auc": best_auc}
